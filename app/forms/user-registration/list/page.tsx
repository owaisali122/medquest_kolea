...
export default function UserRegistrationListPage() {
    ...
    const fetchForms = async () => {
      try {
        ...
        const response = await fetch(
          `/api/forms/pending?formSlug=${encodeURIComponent(USER_REGISTRATION_SLUG)}&allSessions=true`
        )
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-8">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-red-50 border border-red-200 rounded-xl p-6 text-center">
            <h3 className="text-lg font-medium text-red-800 mb-2">Error Loading Registrations</h3>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="mt-1 text-gray-600">List of user registration submissions</p>
          </div>
          <Link
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          {forms.length === 0 ? (
            <div className="px-6 py-12 text-center text-gray-500">
              No registrations yet. Click &quot;Add New&quot; to create one.
            </div>
          ) : (
              ...
              {forms.map((row) => (
                ...
                >
                  ...
                  <div className="col-span-4">
                    ...
                    {row.formSlug && <div className="text-sm text-gray-500">{row.formSlug}</div>}
                  </div>
                  <div className="col-span-3 text-sm text-gray-600">{formatDate(row.lastUpdated)}</div>
                  <div className="col-span-3 flex justify-end gap-2">
                    <Link
                      href={`/forms/user-registration/${row.id}${row.currentStep !== undefined ? `?step=${row.currentStep}` : ''}`}
